...
variable "ci_vm_size" {
  description = "VM size for the CI runner node pool"
  type        = string
  default     = "Standard_D2s_v3"
}

variable "ci_min_count" {
  description = "Minimum node count for CI runner pool (0 = scale-to-zero)"
  type        = number
  default     = 0
}

variable "ci_max_count" {
  description = "Maximum node count for CI runner pool"
  type        = number
  default     = 3
}
