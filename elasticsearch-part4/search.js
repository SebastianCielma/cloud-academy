...
if (ELASTIC_CA_CERT_PATH && fs.existsSync(ELASTIC_CA_CERT_PATH)) {
  ...
  console.warn("UWAGA: Nie znaleziono certyfikatu CA pod ścieżką:", ELASTIC_CA_CERT_PATH);
}
...
async function runSearch() {
  try {
    ...
    const indexExists = await client.indices.exists({ index: ALIAS_NAME });
    ...
    if (indexExists) {
      ...
      console.log(`\nIndex '${ALIAS_NAME}' not found. You might need to run the Publisher logic first in this new environment.`);
    }
