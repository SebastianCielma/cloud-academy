...
def load_config(cloud: str, env: str) -> dict[str, Any]:
    """Load environment config with strict fail-fast validation."""
    env_file = CONFIGS / f"{cloud}.{env}.yaml"
    if not env_file.exists():
        raise ValueError(f"Missing required config file: {env_file}")
    ...
    for section in required_sections:
        if section not in config:
            raise ValueError(f"Missing required '{section}' section in {env_file.relative_to(env_file.parents[1])}")
